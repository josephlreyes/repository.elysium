...
class Generator:
    ...
    def Create_Zips(self,addon_id,version):
        ...
        if not os.path.exists(final_zip):
            ...
            zip.close()
            # Copy over the icon, fanart, addon.xml and changelog.txt to the zip directory
            copyfiles = ['icon.png','fanart.jpg','changelog.txt']
            files = os.listdir(addon_id)
            for file in files:
                if file == 'changelog.txt':
                    shutil.copy(os.path.join(addon_id,file),os.path.join('zips',addon_id,file.replace('.txt', '-{}.txt'.format(version))))
                elif file in copyfiles and file != 'changelog.txt':
    ...
    def _generate_addons_file(self):
        ...
        self._save_file(addons_xml.encode("utf-8"), file=os.path.join('elysium.xml'))

    def _generate_md5_file(self):
        try:
            m = md5.new(open(os.path.join('elysium.xml')).read()).hexdigest()
            self._save_file(m, file=os.path.join('elysium.xml.md5'))
        except Exception, e:
            print "An error occurred creating elysium.xml.md5 file!\n%s" % (e)
